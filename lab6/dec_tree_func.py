...
# Printing a tree of any node degree in a file file_ptr
#   tree - tree represented by array (see addnode)
#   node_no - node number started from 1 (if 0 - no child node)
#   depth_of_node - depth of a node (0 - root, 1 - childs of root, etc.)
# example:
# printnode(file_ptr,tree,1,0) - printing full tree (from root node)
def printnode(file_ptr,tree,node_no,depth_of_node,attribute_value_descr):
    [num_of_rows, num_of_nodes] = tree.shape
    degree = num_of_rows-1
    for j in range(depth_of_node):
        file_ptr.write('---')

    if node_no == 0:
        #file_ptr.write('null \n')
        pass
    elif np.sum(tree[:-1,node_no-1]) == 0:
        file_ptr.write(attribute_value_descr+'class '+ str(tree[-1,node_no-1]) + '\n')
    else:
        file_ptr.write(attribute_value_descr+'node with attribute a'+ str(tree[-1,node_no-1])+'\n')
        for value in range(degree):
            if tree[value,node_no-1] > 0:
                attribute_value_descr = ' a'+str(tree[-1,node_no-1])+' == '+str(value+1)+' -> '
                printnode(file_ptr,tree,tree[value,node_no-1],depth_of_node+1,attribute_value_descr)

def print_tree(file_name,tree):
    f = open(file_name,'w')
    printnode(f,tree,1,0,'')
    f.close()
